...
This call must not write the durable memory note. Composing it here puts it
...
The greeting comes first and is streamed. Nothing the hire never sees may be
...
def stream_session(
    ...
    Strict JSON cannot be streamed as prose -- the first tokens are
    ...
    try:
        for chunk in llm.stream(prompt):
            ...
            if cut != -1:
                ...
                in_greeting = False
                continue
            # A suffix that could still grow into the marker is held back, never
