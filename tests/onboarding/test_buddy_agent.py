...
def test_persona_reaches_the_hire_with_the_tools_it_was_mounted() -> None:
    llm = ScriptedLLMClient(turns=[], answer="hi")

    # The persona is assembled from the tools this hire was actually mounted, so a
    # test about a directive has to mount the tool the directive depends on.
    mounted = [
        _GET_MY_METRICS,
        _tool("get_competencies_to_assess"),
        _tool("record_assessment"),
        _tool("claim_goal"),
    ]

    run_agent_turn([_user("hello")], mounted, llm, StubVectorStore())

    persona = _system_of(llm.chat_calls[0])
    assert "get_competencies_to_assess" in persona
    assert "record_assessment" in persona
...
def test_persona_omits_tools_this_hire_was_not_mounted() -> None:
    ...
    # pull requests. Both of these are genuinely conditional: the backend mounts
    # them only when there is an arrival list, and only while something is unplaced.
...
def test_the_turn_never_folds_however_long_the_window_is() -> None:
    """The turn never folds, and this pins it.

    Folding here would advance the caller's cursor by exactly what was folded, leaving
    the window at its cap forever once it first filled -- so every turn of a long visit
    would pay an extra serialized model call to compress one exchange, ahead of the
    reply the hire is waiting for. `onboarding.buddy_compact` does it afterwards.
