...
The memory fold is not part of opening a visit. What these tests do carry is every
rule the marker machinery needs: it guards ``<<<ACTION>>>``, and a marker arrives one
chunk at a time, so a partial one is indistinguishable from prose until the next
chunk lands.
"""
...
def test_the_open_does_not_rewrite_the_memory_note() -> None:
    """The open writes no memory note.

    Writing one here would compose a hire's durable memory while the model is busy
    greeting them. Folding is `/onboarding/buddy/compact`, and the terminal event
    carries no note at all — a caller cannot persist what it is never handed.
    """
...
def test_holds_back_only_a_possible_marker_prefix() -> None:
    """ "<<<ACT" is both a partial marker and legitimate prose.
...
def test_the_done_greeting_is_exactly_what_was_streamed() -> None:
    """The client renders the tokens; the caller persists ``done``.
